...
namespace FactionColonies
{
        ...
        public static void ProcessEvents(in List<FCEvent> events)
        {
            ...
            for (int i = 0; i < events.Count(); i++)
            {
                ...
                { //Make custom event functions here
                    ...
                    if (temp.def.defName == "enactFactionPolicy")
                    { //enact policy
                        faction.enactPolicy(temp.policy, temp.policySlot);
                        Messages.Message(temp.policy.label + " " + "EnactedInYourFaction".Translate() + "!", MessageTypeDefOf.PositiveEvent);
                    }
                    else
                    ...
                    if (temp.def.defName == "settlementBeingAttacked")
                    {
                        ...
                        } else
                        {
                            ...
                            temp.settlementFCDefending.prosperity -= 20;
                            temp.settlementFCDefending.happiness -= (25 * happinessLostMultiplier);
                            temp.settlementFCDefending.loyalty -= (15 * loyaltyLostMultiplier);
                            ...
                            for (int k = 0; k < 4; k++)
                            {
                                ...
                                if (num >= 7 && temp.settlementFCDefending.buildings[k].defName != "Empty" && temp.settlementFCDefending.buildings[k].defName != "Construction" )
                                {
    ...
    public class FCEvent : IExposable, ILoadReferenceable
    {
        ...
        public void ExposeData()
        {
            ...
            Scribe_Values.Look<int>(ref policySlot, "policySlot");

            Scribe_Defs.Look<BuildingFCDef>(ref building, "building");
            Scribe_Defs.Look<PolicyFCDef>(ref policy, "policy");
        ...
        public int policySlot = -1;
        public PolicyFCDef policy = null;
